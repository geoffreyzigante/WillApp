...
function HomeScreen({ events, favorites, toggleFav, onOpenEvent, onOpenSelfie, onOpenOrg, search, setSearch, tab, setTab }) {
  ...
  return (
    <ScrollView style={s.scroll} contentContainerStyle={{ paddingBottom: 120 }} showsVerticalScrollIndicator={false}>
      ...
      <TouchableOpacity style={s.searchBtn} activeOpacity={0.85}>
        ...
        <TextInput
          placeholder="Trouver mon événement"
          placeholderTextColor="#FFFFFF"
          value={search}
          onChangeText={setSearch}
          style={s.searchInputBtn}
        />
      </TouchableOpacity>
      ...
      {filtered.length === 0 ? (
        ...
        filtered.map((event) => (
          <EventCard
            key={event.code}
            event={event}
            isFav={favorites.includes(event.code)}
            onPress={() => onOpenEvent(event)}
            onFav={() => toggleFav(event.code)}
          />
        ))
      )}
    </ScrollView>
...
function EventCard({ event, isFav, onPress, onFav }) {
  ...
  return (
    <TouchableOpacity activeOpacity={0.9} onPress={onPress} style={s.eventCard}>
      ...
      <LinearGradient
        ...
        style={StyleSheet.absoluteFillObject}
      />
      <TouchableOpacity style={s.heartBtn} onPress={onFav} hitSlop={10}>
        <Icon.Heart filled={isFav} color="#FFFFFF" size={22} />
      </TouchableOpacity>
...
function EventDetailScreen({ event, onClose, onOpenSelfie }) {
  const [kmFilter, setKmFilter] = useState('km1');
  const tint = TYPE_COLORS[event.event_type] || TYPE_COLORS.Autre;
  ...
  return (
    <ScrollView style={s.scroll} contentContainerStyle={{ paddingBottom: 120 }} showsVerticalScrollIndicator={false}>
      ...
      {/* Emplacement */}
      <View style={s.empRow}>
        <Text style={s.sectionTitle}>Emplacement</Text>
        <View style={s.pillRow}>
          {['km1', 'km13'].map(k => (
            <TouchableOpacity key={k} style={[s.pill, kmFilter === k && s.pillActive]} onPress={() => setKmFilter(k)}>
              <Text style={[s.pillText, kmFilter === k && s.pillTextActive]}>{k}</Text>
            </TouchableOpacity>
          ))}
        </View>
      </View>
...
function LoginModal({ visible, role, events, onClose, onSuccess }) {
  ...
  const submit = async () => {
    if (!code || !password) return Alert.alert('Champs requis', 'Code et mot de passe.');
    if (role === 'photographer' && !name) return Alert.alert('Champ requis', 'Ton prénom.');
    setBusy(true);
...
export default function App() {
  ...
  useEffect(() => {
    ...
    AsyncStorage.getItem('@will_favorites').then(v => v && setFavorites(JSON.parse(v)));
  ...
  const toggleFav = useCallback((code) => {
    setFavorites(prev => {
      ...
      AsyncStorage.setItem('@will_favorites', JSON.stringify(next));
      return next;
    });
  }, []);
